...
if (isset($_POST["upload"])) {
    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
        $targetPath = '../public/uploads/sys_data/' . 'Departments_bulk_import_' . time() . '_' . $_FILES['file']['name'];
        ...
        for ($i = 1; $i <= $sheetCount; $i++) {
            ...
            if (isset($spreadSheetAry[$i][0])) {
                $department_code = mysqli_real_escape_string($conn, $spreadSheetAry[$i][0]);
            }

            $department_name  = "";
            if (isset($spreadSheetAry[$i][1])) {
                $department_name  = mysqli_real_escape_string($conn, $spreadSheetAry[$i][1]);
            }

            $department_details  = "";
            if (isset($spreadSheetAry[$i][2])) {
                $department_details  = mysqli_real_escape_string($conn, $spreadSheetAry[$i][2]);
            }
            ...
            $log_type = 'Bulk Imported Departtments To : ' . $faculty_code . ' ' . $faculty_name;
            ...
            if (mysqli_num_rows($res) > 0) {
                ...
                if ($department_code == $row['department_code']) {
                    $err = 'Department With This Code: ' . $department_code . ' Alreaady Exists';
                }
            } else {
                /* Persist Bulk Imports If No Duplicates */
                if (!empty($department_code) || !empty($department_name)) {
                    $query = "INSERT INTO departments (department_code, department_name, department_faculty_id, department_user_id, department_details) VALUES(?,?,?,?,?)";
