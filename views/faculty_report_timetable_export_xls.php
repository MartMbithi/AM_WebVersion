...
academic_calendar JOIN faculties 
...
while ($sys = $res->fetch_object()) {
    ...
    /* Excel File Name */
    $fileName = $sys->faculty_name . "  " . $sys->calendar_year . ' ' . $sys->calendar_semester . " Overall Timetable.xls";

    /* Excel Column Name */
    $header = array($sys->faculty_name . ' ' . $sys->calendar_year  . ' ' . $sys->calendar_semester . ' Overall Timetable');
    $fields = array('Module Details', 'Lecturer', 'Class Room Number', 'Day', 'Time');
    ...
    $query = $mysqli->query("SELECT * FROM timetable t
    ...
    WHERE ma.allocation_calendar_id = '$session'
    AND d.department_faculty_id = '$view'");
    if ($query->num_rows > 0) {
        /* Load All Fetched Rows */
        while ($row = $query->fetch_assoc()) {
            $lineData = array(
                $row['module_code'] . ' ' . $row['module_name'], $row['user_number'] . ' ' . $row['user_name'],
                $row['timetable_class_room'], $row['timetable_class_day'], $row['timetable_class_start_time'] . ' To ' . $row['timetable_class_end_time'],
            );
            array_walk($lineData, 'filterData');
            $excelData .= implode("\t", array_values($lineData)) . "\n";
        }
    } else {
        $excelData .= 'No Timetable Records Available...' . "\n";
    }
