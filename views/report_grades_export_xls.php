...
while ($sys = $res->fetch_object()) {
    ...
    /* Excel File Name */
    $fileName = $sys->system_name . "  " . $sys->user_number . ' ' . $sys->user_name . " Perfomances.xls";

    /* Excel Column Name */
    $header = array($sys->system_name . ' ' . $sys->user_number  . ' ' . $sys->user_name . ' Perfomance');
    $fields = array('Module Details', 'Cumulative Marks', 'Grade Attained');
    ...
    $query = $mysqli->query("SELECT * FROM student_grades sg
    ...
    if ($query->num_rows > 0) {
        ...
        while ($row = $query->fetch_assoc()) {
            /* Grading */
            $workScore = $row['grade_marks_attained'];
            switch ($workScore) {
            ...
            $lineData = array(
                $row['module_code'] . ' ' . $row['module_name'], $row['grade_marks_attained'], $grade
            );
            array_walk($lineData, 'filterData');
            $excelData .= implode("\t", array_values($lineData)) . "\n";
        }
    } else {
        $excelData .= 'No Perfomance Records Available...' . "\n";
    }
