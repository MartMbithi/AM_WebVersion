...
JOIN faculties WHERE faculty_id = '$view' ";
...
while ($sys = $res->fetch_object()) {
    ...
    /* Excel File Name */
    $fileName = $sys->faculty_name . " Department Reports.xls";

    /* Excel Column Name */
    $header = array($sys->faculty_name . ' Departments');
    $fields = array('DEPARTMENT CODE', 'DEPARTMENT NAME', 'DEPARTMENT HEAD');
    ...
    if ($query->num_rows > 0) {
        ...
        while ($row = $query->fetch_assoc()) {
            $lineData = array($row['department_code'], $row['department_name'], $row['user_number'] . '  ' . $row['user_name']);
