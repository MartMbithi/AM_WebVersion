...
if (isset($_POST['delete'])) {
    ...
    /* Persist */
    $sql = "DELETE FROM requisitions WHERE requisition_id =?";
    $prepare = $mysqli->prepare($sql);
    $bind = $prepare->bind_param('s', $requisition_id);
    $prepare->execute();
    if ($prepare) {
        $success = "Deleted";
    } else {
                        ...
                        <div class="page-title-box">
                            <div class="float-right">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">Manage Requisitions</li>
                                </ol>
                            </div>
                            <h4 class="page-title">My Requisitions</h4>
                        </div>
                        <hr>
                    </div>
                    <!--end col-->
                    ...
                    <div class="col-lg-12">
                        <div class="">
                            <div class="col-lg-12">
                                ...
                                $ret = "SELECT * FROM requisitions WHERE requisition_user_id = '$user_id'
                                ORDER BY requisition_created_at DESC ";
                                $stmt = $mysqli->prepare($ret);
                                $stmt->execute(); //ok
                                $res = $stmt->get_result();
                                $cnt = 1;
                                while ($requisition = $res->fetch_object()) {
                                ?>
                                    <div class="card col-md-12 col-xl-12 col-lg-12 col-sm-12">
                                        <div class="row g-0">
                                            <div class="col-md-12">
                                                <div class="card-body">
                                                    <h5 class="card-title">Requisition # <?php echo $cnt; ?> - <small class="text-primary">Posted On: <?php echo date('d M Y g:ia', strtotime($requisition->requisition_created_at)); ?></small></h5>
                                                    <?php echo $requisition->requisition_details; ?>
                                                    <div class="float-left ">Progress: <small class="text-muted ml-3 font-13"><?php echo $requisition->requisition_progress; ?>%</small></div><br>
                                                    <div class="progress mt-2" style="height:3px;">
                                                        <div class="progress-bar bg-pink" role="progressbar" style="width: <?php echo $requisition->requisition_progress; ?>%; border-radius:5px;" aria-valuenow="<?php echo $requisition->requisition_progress; ?>" aria-valuemin="0" aria-valuemax="100"></div>
                                                    </div>
                                                    <hr>
                                                    <form method="post">
                                                        <div class="text-right">
                                                            <input type="hidden" name="requisition_id" value="<?php echo $requisition->requisition_id; ?>">
                                                            <button class="btn btn-outline-danger btn-sm" name="delete" type="submit"><i class="fas fa-trash"></i> Delete</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
