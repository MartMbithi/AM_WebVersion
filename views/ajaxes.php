...
if (!empty($_POST["UserID"])) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo htmlentities($row['user_number']);
    }
}
...
if (!empty($_POST["UserNumber"])) {
    ...
    $stmt = $DB_con->prepare("SELECT * FROM users WHERE  user_id = :id");
    $stmt->execute(array(':id' => $user_number));
...
if (!empty($_POST["UserEmail"])) {
    ...
    $stmt = $DB_con->prepare("SELECT * FROM users WHERE  user_id = :id");
    $stmt->execute(array(':id' => $user_email));
...
if (!empty($_POST["ModuleID"])) {
    ...
    $stmt = $DB_con->prepare("SELECT * FROM modules WHERE  module_id = :id");
    $stmt->execute(array(':id' => $module_id));

    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        echo htmlentities($row['module_code'] . '  ' . $row['module_name']);
    }
}
