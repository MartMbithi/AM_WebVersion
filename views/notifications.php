...
if (isset($_POST['mark_as_read'])) {
    ...
    /* Update Notificatkion Status */
    $sql = "UPDATE notifications SET notification_status = ? WHERE  notification_user_id = ?";
...
if (isset($_POST['delete_all'])) {
    ...
    /* Delete */
    $sql = "DELETE FROM notifications WHERE notification_user_id =?";
    $prepare = $mysqli->prepare($sql);
    $bind = $prepare->bind_param('s', $notification_user_id);
    $prepare->execute();
    if ($prepare) {
                                ...
                                <div class="card <?php echo $border; ?>">
                                    <div class="card-body">
                                        <div class="media-body align-self-center">
                                            <h4 class="mt-0 mb-1 title-text text-dark"><?php echo date('d M Y g:ia', strtotime($notification->notification_created_at)); ?></h4>
                                            <p class="text-muted mb-1"><?php echo  $notification->notification_details; ?></p>
                                        </div>
                                    </div>
                                </div>
                            <?php } ?>
