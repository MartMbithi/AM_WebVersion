...
while ($sys = $res->fetch_object()) {
    ...
    /* Excel File Name */
    $fileName = $sys->system_name . " Faculties Reports.xls";

    /* Excel Column Name */
    $header = array($sys->system_name . ' Department Reports');
    $fields = array('DEPARTMENT CODE', 'DEPARTMENT NAME', 'DEPARTMENT HEAD', 'FACULTY DETAILS');
    ...
    $query = $mysqli->query("SELECT * FROM departments d
    ...
    if ($query->num_rows > 0) {
        ...
        while ($row = $query->fetch_assoc()) {
            $lineData = array($row['department_code'], $row['department_name'], $row['user_number'] . '  ' . $row['user_name'], $row['faculty_code'] . '  ' . $row['faculty_name']);
            array_walk($lineData, 'filterData');
            $excelData .= implode("\t", array_values($lineData)) . "\n";
        }
    } else {
        $excelData .= 'No Department Records Available...' . "\n";
    }
