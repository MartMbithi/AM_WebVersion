...
WHERE calendar_id = '$session' AND faculty_id = '$view' ";
...
while ($sys = $res->fetch_object()) {
    ...
    /* Excel File Name */
    $fileName = $sys->faculty_name . "  " . $sys->calendar_year . ' ' . $sys->calendar_semester . " Teaching Allocations.xls";

    /* Excel Column Name */
    $header = array($sys->faculty_name . ' ' . $sys->calendar_year  . ' ' . $sys->calendar_semester . ' Module Teaching Allocations');
    $fields = array('Allocated Lecturer', 'Module Details');
    ...
    $query = $mysqli->query("SELECT * FROM module_allocations ma
    ...
    WHERE ac.calendar_id = '$session' 
    AND d.department_faculty_id = '$view'");
    if ($query->num_rows > 0) {
        /* Load All Fetched Rows */
        while ($row = $query->fetch_assoc()) {
            $lineData = array($row['user_number'] . ' ' . $row['user_name'], $row['module_code'] . ' ' . $row['module_name']);
            array_walk($lineData, 'filterData');
            $excelData .= implode("\t", array_values($lineData)) . "\n";
        }
    } else {
        $excelData .= 'No Teaching Allocation Records Available...' . "\n";
    }
