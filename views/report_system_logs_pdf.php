...
while ($sys = $res->fetch_object()) {
    ...
    <html>
        ...
        <body style="margin:1px;">
            <div class="footer">
                <hr>
                <i>' . $sys->system_name . '  Access Logs ' . date('d M Y') . '</i>
            </div>
            ...
            <h3 class="list_header" align="center">
                ...
                <h4> System Access Logs From ' . $start . ' To ' . $end . ' </h4>
            </h3>
            <table border="1" cellspacing="0" width="98%" style="font-size:9pt">
            <thead>
                <tr>
                    <th style="width:30%">IP Address</th>
                    <th style="width:100%">User Details</th>
                    <th style="width:100%">Log Details</th>
                    <th style="width:90%">Created On</th>
                </tr>
            </thead>
            ';
            ...
            while ($logs = $res->fetch_object()) {
                ...
                $html .=
                    '
                        <tr>
                            <td>' . $logs->log_ip . '</td>
                            <td>' . $logs->user_number . ' ' . $logs->user_name . '</td>
                            <td>' . $logs->log_type . '</td>
                            <td>' . date('d M Y g:ia', strtotime($logs->log_created_on)) . '</td>
                        </tr>
                        ';
            }
            $html .= '
    ...
    $dompdf->render();
    $dompdf->stream($sys->system_name . ' Access Logs From ' . $start . ' To ' . $end, array("Attachment" => 1));
