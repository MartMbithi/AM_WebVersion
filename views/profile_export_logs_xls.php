...
while ($sys = $res->fetch_object()) {
    ...
    /* Excel File Name */
    $fileName = $sys->user_number . ' ' . $sys->user_name . " Access Logs.xls";

    /* Excel Column Name */
    $header = array($sys->system_name . ' ' . $sys->user_number . ' ' . $sys->user_name .  ' Access Logs');
    $fields = array('IP Address', 'Log Details', 'Date Created');
    ...
    if ($query->num_rows > 0) {
        ...
        while ($row = $query->fetch_assoc()) {
            ...
            $lineData = array($row['log_ip'], $row['log_type'], $log_date);
