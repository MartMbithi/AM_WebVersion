...
if (isset($_POST['add_req'])) {
    $requisition_user_id = $_SESSION['user_id'];
    $requisition_details = $_POST['requisition_details'];
    $requisition_progress = '0';

    /* Persist */
    $sql = "INSERT INTO  requisitions (requisition_user_id, requisition_details, requisition_progress) VALUES(?,?,?)";
    $prepare = $mysqli->prepare($sql);
    $bind = $prepare->bind_param(
        'sss',
        $requisition_user_id,
        $requisition_details,
        $requisition_progress
    );
    $prepare->execute();
    if ($prepare) {
        $success = "Your Requisition Has Been Submitted";
    } else {
        $err = "Failed!, Please Try Again Requisition";
    }
}
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="page-title-box">
                            ...
                            <h4 class="page-title">Add Requisition</h4>
                        </div>
                        <!--end page-title-box-->
                    </div>
                    <!--end col-->
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="form-group col-md-12">
                                                    <label for="">Requisition Details</label>
                                                    <textarea type="text" rows="5" required name="requisition_details" class="summernote form-control"></textarea>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="text-right">
                                            <button type="submit" name="add_req" class="btn btn-primary">Submit</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
