 ...
 * You may not make the Soft ware available on a network, or in any way provide the Software to multiple users
...
while ($sys = $res->fetch_object()) {
    ...
    <html>
        ...
        <body style="margin:1px;">
            <div class="footer">
                <hr>
                <i>' . $sys->system_name . ' ' . $sys->user_number . ' ' . $sys->user_name . ' Perfomances.  Generated On ' . date('d M Y') . '</i>
            </div>
            ...
            <h3 class="list_header" align="center">
                ...
                <h4>' . $sys->user_number . '  ' . $sys->user_name . ' Perfomances </h4>
            </h3>
            ...
            WHERE sg.grade_marks_user_id = '$user'";
            $stmt = $mysqli->prepare($ret);
            $stmt->execute(); //ok
            $res = $stmt->get_result();
            while ($tt = $res->fetch_object()) {
                /* Grading System */
            $workScore = $tt->grade_marks_attained;
            switch ($workScore) {
    ...
    $dompdf->render();
    $dompdf->stream($sys->system_name . ' ' . $sys->user_number . ' ' . $sys->user_name . ' Perfomances', array("Attachment" => 1));
