...
while ($sys = $res->fetch_object()) {
    ...
    /* Excel File Name */
    $fileName = $sys->system_name . " Students.xls";

    /* Excel Column Name */
    $header = array($sys->system_name . ' Students Reports');
    $fields = array('Admission Number', 'Full Name', 'Email', 'Phone Number', 'Address');
    ...
    if ($query->num_rows > 0) {
        ...
        while ($row = $query->fetch_assoc()) {
            ...
            $lineData = array($row['user_number'], $row['user_name'], $row['user_email'], $row['user_phone'], $row['user_adr']);
            array_walk($lineData, 'filterData');
            $excelData .= implode("\t", array_values($lineData)) . "\n";
        }
    } else {
        $excelData .= 'No Students Records Available...' . "\n";
    }
